 ...
 * In Keycloak, sets up a mapper that includes the users roles in the /userinfo endpoint
 */
...
export const authConfig: AuthConfig = {
  ...
  redirectUri: getSilentRefreshUrl(),
...
export class AuthNService {
  ...
  public async getUserProfile(): Promise<UserProfile | null> {
    ...
    return await firstValueFrom(
      this.isLoggedIn.pipe(filter((isLoggedIn) => isLoggedIn !== undefined)).pipe(
        map((_) => {
          return this.userProfile;
        })
      )
